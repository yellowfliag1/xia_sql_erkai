...
public class BurpExtender extends AbstractTableModel implements IBurpExtender, ITab, IHttpListener,IScannerCheck, IMessageEditorController,IContextMenuFactory
{
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks)
    {
        ...
        this.stdout.println("hello xia sql erkai!");
        this.stdout.println("你好 欢迎使用 瞎注魔改版!");
        this.stdout.println("version:erkai");
        ...
        callbacks.setExtensionName("xia SQL erkai");
        ...
        SwingUtilities.invokeLater(new Runnable()
        {
            ...
            public void run()
            {
                ...
                JLabel jls=new JLabel("插件名：瞎注魔改版 author：X");    //创建一个标签
                JLabel jls_1=new JLabel("blog： NULL");    //创建一个标签
                JLabel jls_2=new JLabel("版本：xia SQL erkai");    //创建一个标签
                JLabel jls_3=new JLabel("感谢名单：Moonlit、阿猫阿狗、Shincehor、算命縖子");    //创建一个标签
                ...
                JCheckBox chkbox8=new JCheckBox("测试Cookie(UA)");    //创建指定文本的复选框
                JCheckBox chkbox9=new JCheckBox("开启UA测试(需要先勾选Cookie)");
                ...
                chkbox1.addItemListener(new ItemListener() {
                    ...
                    public void itemStateChanged(ItemEvent e) {
                        if(chkbox1.isSelected()){
                            stdout.println("插件xia SQl erkai启动");
                            switchs = 1;
                        }else {
                            stdout.println("插件xia SQL erkai关闭");
                            switchs = 0;
                        }
                ...
                chkbox9.addItemListener(new ItemListener() {
                    ...
                    public void itemStateChanged(ItemEvent e) {
                        if(chkbox9.isSelected()) {
                            stdout.println("启动 测试UA");
                            is_ua = 2;
                        }else {
                            stdout.println("关闭 测试UA");
                            is_ua = -1;
                        }
                    }
                });
    ...
    public List<JMenuItem> createMenuItems(final IContextMenuInvocation invocation) {
        ...
        if(invocation.getToolFlag() == IBurpExtenderCallbacks.TOOL_REPEATER || invocation.getToolFlag() == IBurpExtenderCallbacks.TOOL_PROXY){
            ...
            JMenuItem jMenu = new JMenuItem("Send to xia SQL erkai");
            ...
            jMenu.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    if(switchs == 1) {
                        ...
                        BurpExtender.this.stdout.println("插件xia SQL erkai关闭状态！");
                    }
            ...
            if (para.getType() == 0 || para.getType() == 1 || para.getType() == 6 || para.getType() == is_cookie){ //getTpe()就是来判断参数是在那个位置的
                ...
                for (String payload : payloads) {
                    ...
                    if(para.getType() == 6){
                        ...
                        byte[] request = helpers.updateParameter(new_Request, newPara);//更新请求包的参数
//                        int bodyOffset = helpers.analyzeRequest(baseRequestResponse).getBodyOffset();
                        boolean modify = false;
                        if (is_ua == 2 && para.getType() == is_cookie){
                            int oldBodyOffset = helpers.analyzeRequest(baseRequestResponse).getBodyOffset();
                            byte[] oldBody = Arrays.copyOfRange(new_Request, oldBodyOffset, new_Request.length);
                            // 修改User-Agent头
                            for (int i = 0; i < headers.size(); i++) {
                                String header = headers.get(i);
                                if (header.startsWith("User-Agent:")) {
                                    String newUserAgent = header + payload;
                                    stdout.println("payload是"+payload);
                                    headers.set(i, newUserAgent);
                                    break; // 假设只有一个User-Agent头，找到后即可退出循环
                                }
                            }
                            // 重新构建最终的新的HTTP消息
                            byte[] newRequest = helpers.buildHttpMessage(headers, oldBody);
                            requestResponse = callbacks.makeHttpRequest(iHttpService, newRequest); //发送请求
                            modify = true;
                        }
                        if (!modify){
                            time_1 = (int) System.currentTimeMillis();
                            requestResponse = callbacks.makeHttpRequest(iHttpService, request);//发送请求
                            time_2 = (int) System.currentTimeMillis();
                        }
